...
def run_consensus_annotation(
    ...
    *,
    ...
    # Stable identifiers for the consensus call. Purely additive: the label columns are
    # untouched, but the exported h5ad now carries a Cell Ontology id alongside the
    # display string, so it can be joined against CELLxGENE / Azimuth / HCA resources
    # instead of matched on spelling. Unresolvable labels get "" rather than a guess.
    per_cluster_columns.update(tools.ontology_maps(consensus_label))
    if cfg.enable_singler:
