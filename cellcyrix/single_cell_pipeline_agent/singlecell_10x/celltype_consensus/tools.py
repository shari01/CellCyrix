...
from .model_provenance import record_validated_model
...
def _celltypist_per_cell_labels(
    ...
    if bundled:
        ...
        # Most of the bundled models were pickled by scikit-learn 0.24.1 and we run a
        # far newer one, which makes sklearn warn that results "might be invalid".
        # Prove they are not: recompute predict_proba from the persisted arrays and
        # require an exact match. Also outside the try/except, and for the same
        # reason — a model whose inference cannot be reproduced must abort the run
        # rather than quietly annotate with it.
        record_validated_model(bundled)
    try:
        if bundled:
...
#: Value written when a label cannot be placed in the hierarchy, so an ontology column
#: never silently implies "no term exists" where the truth is "we could not resolve it".
ONTOLOGY_UNRESOLVED = ""
...
def ontology_maps(
    ...
    """Cell Ontology IDs and canonical names for a per-cluster label map.

    The resolver already computes ``cl_id``, ``node_id`` and ``canonical_label`` for
    every label it places, and the pipeline was discarding all three — the consensus
    call reached ``obs`` as a display string with no stable identifier behind it. That
    makes an exported ``h5ad`` hard to join against any other dataset, because
    "Natural killer cell" is a spelling and ``CL:0000623`` is an identity.

    Emitting them costs one cached resolver lookup per cluster and makes the output
    interoperable with Cell Ontology-indexed resources (CELLxGENE, Azimuth references,
    the HCA) without changing a single label the pipeline decided on.

    Args:
        label_by_cluster: Cluster id -> consensus label, as decided upstream.

    Returns:
        ``{"celltype_cl_id": {...}, "celltype_ontology_node_id": {...},
        "celltype_canonical_label": {...}}`` — three per-cluster maps ready for
        :func:`broadcast_and_validate`. Unresolvable labels map to
        :data:`ONTOLOGY_UNRESOLVED`, never to a guessed term.
    """
    cl_ids: Dict[str, str] = {}
    node_ids: Dict[str, str] = {}
    canonical: Dict[str, str] = {}
    for cluster, label in label_by_cluster.items():
        resolution = _resolve_cached(str(label or "").strip())
        if resolution is None:
            cl_ids[cluster] = ONTOLOGY_UNRESOLVED
            node_ids[cluster] = ONTOLOGY_UNRESOLVED
            canonical[cluster] = ONTOLOGY_UNRESOLVED
            continue
        cl_ids[cluster] = str(getattr(resolution, "cl_id", "") or ONTOLOGY_UNRESOLVED)
        node_ids[cluster] = str(
            getattr(resolution, "node_id", "") or ONTOLOGY_UNRESOLVED
        )
        canonical[cluster] = str(
            getattr(resolution, "canonical_label", "") or ONTOLOGY_UNRESOLVED
        )

    n_with_cl = sum(1 for value in cl_ids.values() if value)
    logger.info(
        "[ONTOLOGY] %d/%d clusters carry a Cell Ontology id.", n_with_cl, len(cl_ids)
    )
    return {
        "celltype_cl_id": cl_ids,
        "celltype_ontology_node_id": node_ids,
        "celltype_canonical_label": canonical,
    }
