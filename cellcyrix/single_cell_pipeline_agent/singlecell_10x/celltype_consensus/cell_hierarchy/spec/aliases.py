...
# ---------------------------------------------------------------------------
# cell_ontology — the full Cell Ontology term names, as written by CELLxGENE.
#
# Added after the first real benchmark run. CELLxGENE (and therefore every dataset
# pulled from it, including atlases published in Cell) labels cells with the complete
# CL term name rather than a working abbreviation: "central memory CD4-positive,
# alpha-beta T cell" instead of "CD4 TCM". None of those forms were in this crosswalk,
# so on AIDA v1 the resolver rejected 15 of 32 distinct labels covering 6,465 of 11,385
# cells — 57% of the ground truth silently dropped from scoring, and, worse, 57% of any
# real CELLxGENE-derived input silently unannotatable.
#
# normalize() cannot rescue these on its own: it collapses case, punctuation and
# plurals, but "naive thymus-derived CD8-positive, alpha-beta T cell" and
# "CD8 naive T cell" share no token order, so only an explicit crosswalk connects them.
#
# Granularity note: these map to the FINEST node the tree actually has. Where the tree
# has no such subtype (CD1c+/CD141+ myeloid DC, double-negative Treg) the label maps to
# its parent rather than to a new node — inventing tree nodes to match one vocabulary
# would change the pipeline's output space, which is not what a naming crosswalk is for.
_CELL_ONTOLOGY_ALIASES: Dict[str, List[str]] = {
    # --- CD4 T ---
    "cd4_t_cell": [
        "CD4-positive, alpha-beta T cell",
        "CD4-positive, alpha-beta T cells",
        "CD4-positive alpha-beta T cell",
    ],
    "cd4_naive_t_cell": [
        "naive thymus-derived CD4-positive, alpha-beta T cell",
        "naive CD4-positive, alpha-beta T cell",
    ],
    "cd4_central_memory_t_cell": [
    ...
    "cd8_t_cell": [
        ...
        "CD8-positive, alpha-beta T cells",
        "CD8-positive alpha-beta T cell",
        # The tree has no single "CD8 cytotoxic" node; cytotoxicity is a state, and
        # effector CD8 cells are the default reading of this CL term.
        "CD8-positive, alpha-beta cytotoxic T cell",
    ],
    "cd8_naive_t_cell": [
        "naive thymus-derived CD8-positive, alpha-beta T cell",
        "naive CD8-positive, alpha-beta T cell",
    ],
    "cd8_central_memory_t_cell": [
    ...
    "cd8_effector_memory_t_cell": [
        ...
        # CL's generic "CD8 memory" has no central/effector split; effector memory is
        # the larger population in blood, so it is the least-wrong single target.
        "CD8-positive, alpha-beta memory T cell",
    ],
    # --- other T ---
    "regulatory_t_cell": [
        "CD4-positive, CD25-positive, alpha-beta regulatory T cell",
        # No double-negative Treg node exists; the parent is the honest target.
        "double negative T regulatory cell",
    ],
    "gamma_delta_t_cell": ["gamma-delta T cell"],
    "mait_cell": ["mucosal invariant T cell", "mucosal-associated invariant T cell"],
    # --- NK ---
    "cd56_dim_nk_cell": [
    ...
    "intermediate_monocyte": ["CD14-positive, CD16-positive monocyte"],
    "non_classical_monocyte": [
        "CD14-low, CD16-positive monocyte",
        "CD14-negative, CD16-positive monocyte",
    ],
    # --- dendritic ---
    # The tree has one conventional-DC node, so both cDC1 (CD141+) and cDC2 (CD1c+)
    # land there rather than on invented subtypes.
    "dendritic_cell": [
    ...
    "b_cell": ["mature B cell"],
    # --- other ---
    "innate_lymphoid_cell": ["innate lymphoid cell"],
    "platelet": ["platelet"],
