...
def _celltypist_model_provenance() -> dict:
    """Provenance for the CellTypist models this run loaded and validated.

    Most of the bundled models were pickled by an older scikit-learn than the one that
    runs them, so each is checked at load time by recomputing ``predict_proba`` from
    its persisted arrays (see
    :mod:`~.celltype_consensus.model_provenance`). This surfaces both the skew and the
    check that clears it in ``provenance/manifest.json``.

    Returns:
        The provenance block, or a reason string when CellTypist was never loaded (an
        offline run with the voter disabled), so the manifest key is always present.
    """
    try:
        from .celltype_consensus.model_provenance import run_provenance

        record = run_provenance()
    except Exception as exc:  # noqa: BLE001 - provenance must never fail a run
        logger.debug("celltypist model provenance unavailable: %r", exc)
        return {"unavailable": repr(exc)}
    if not record.get("models"):
        record["note"] = "no bundled CellTypist model was loaded in this run"
    return record


def _llm_cache_stats() -> dict:
    """LLM response-cache counters for this run, for ``provenance/manifest.json``.

    Returns:
        The cache stats block, or a reason string when the cache module is
        unavailable, so the manifest key is always present.
    """
    try:
        from .celltype_consensus import llm_cache

        return llm_cache.stats()
    except Exception as exc:  # noqa: BLE001 - provenance must never fail a run
        logger.debug("llm cache stats unavailable: %r", exc)
        return {"unavailable": repr(exc)}
    ...
    try:
        manifest_path = write_run_manifest(
            ...
            extra={
                ...
                # Which scikit-learn pickled each CellTypist model this run loaded,
                # against the one that ran it, plus the numeric equivalence proof.
                # Most bundled models predate the runtime sklearn, so recording the
                # skew AND the check that clears it is what makes the annotation
                # numbers auditable rather than merely warning-free.
                "celltypist_model_provenance": _celltypist_model_provenance(),
                # How much of this run's LLM output was replayed from cache versus
                # fetched. `fully_cached: true` means the run made no LLM network call
                # and can be regenerated exactly from the cache directory alone.
                "llm_response_cache": _llm_cache_stats(),
            },
        )
