...
from agentic_ai_wf.llm.tenant_ids import (
...
def test_scoped_thread_id_no_org_legacy(monkeypatch, caplog):
    monkeypatch.setattr(
        "agentic_ai_wf.llm.tenant_ids._strict_thread_ids", lambda: False
    )
    reset_scoped_thread_warning_for_tests()
...
def test_scoped_thread_id_strict_missing_org(monkeypatch):
    monkeypatch.setattr("agentic_ai_wf.llm.tenant_ids._strict_thread_ids", lambda: True)
