...
def _chat(cfg: ConsensusConfig, system: str, user: str) -> str:
    """One chat completion via OpenRouter. Retries transport errors; raises
    OpenRouterError when retries are exhausted. Returns the message content."""
    try:
        import requests
    ...
    body = {
        ...
        ),  # bound reply so JSON can't truncate
        "messages": [
